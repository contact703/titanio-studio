...
def calculate_importance(text: str, layer: str, source: str) -> float:
    """Score importance: balanced across layers, with content bonuses."""
    # Reduced gap between layers — similarity should matter more than layer
    base = {"semantic": 1.2, "episodic": 1.0, "reference": 0.85}.get(layer, 1.0)
    ...
    if date_match:
        try:
            ...
            recency = max(0.6, 1.0 - (days_ago * 0.015))  # Decay 1.5% per day, floor 0.6
            base *= recency
        except:
            pass
    
    # Content importance signals (reduced multipliers)
...
def do_search(query: str, top_k: int = 5, layer_filter: str = None):
    ...
    for row in rows:
        ...
        # Final score = similarity^1.5 * importance^0.5
        # Similarity weighs MORE than importance — the most similar result should win
    ...
    # Filter out low-quality results (similarity < 0.5 = probably irrelevant)
    results = [r for r in results if r["similarity"] >= 0.45]
    
    # Diversity boost: avoid same source dominating results
    # Keep max 2 results from same source file
    seen_sources = {}
    diverse_results = []
    for r in results:
        src = r["source"]
