# 🤖 AGENTE HELBER — Setup e Configuração
...
```bash
mkdir -p ~/titanio-dashboard/memoria-especialistas/helber-agent/{sessions,prompts}
```

### 2. Criar arquivos de memória
```bash
# context.md — instruções do agente
cat > ~/titanio-dashboard/memoria-especialistas/helber-agent/context.md << 'EOF'
# Helber Agent 🤖

**Especialidade:** Assistente pessoal de Helber
...
### 3. Criar memory.json
```bash
cat > ~/titanio-dashboard/memoria-especialistas/helber-agent/memory.json << 'EOF'
{
  "specialistId": "helber-agent",
  "specialistName": "Helber Agent",
  "version": 1,
...
```bash
cat > ~/titanio-dashboard/memoria-especialistas/helber-agent/lessons.json << 'EOF'
{
  "specialistId": "helber-agent",
  "lessonsLearned": [
    {
...
```bash
# Adicionar:
HELBER_AGENT_ID=helber-agent
HELBER_AGENT_ENABLED=true
HELBER_AGENT_MEMORY_PATH=/memoria-especialistas/helber-agent/
```
...
### 1. Via API
```bash
curl -X POST http://localhost:4444/api/squad/helber-agent/task \
...
- Tab "Especialistas"
- Procurar "Helber Agent"
- Clicar em "Executar tarefa"

### 3. Via Chat
```
"Helber Agent, responda: Quantos especialistas temos?"
```
...
### Acessar histórico
```bash
cat ~/titanio-dashboard/memoria-especialistas/helber-agent/memory.json | jq .recentTasks
```

### Limpar memória (se necessário)
```bash
rm -rf ~/titanio-dashboard/memoria-especialistas/helber-agent/sessions/*
# Mas manter memory.json e lessons.json!
```

---

## 🔄 SINCRONIZAR COM CONTACT & TIAGO
...
git add memoria-especialistas/helber-agent/
git commit -m "Helber Agent: atualizar memória"
...
curl http://localhost:4444/api/squad/helber-agent | jq .

# Ver lessons aprendidas
cat ~/titanio-dashboard/memoria-especialistas/helber-agent/lessons.json | jq .lessonsLearned
```
